...
/*   Updated: 2023/08/19 10:56:54 by bbouagou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*clean_stash(char *stash)
{
	...
	if (stash[i] != '\0')
	{
		new_stash = (char *)malloc(ft_len(stash) - i);
		if (new_stash == NULL)
			return (NULL);
		copy(new_stash, stash + i + 1, ft_len(stash) - i - 1);
		free (stash);
		return (new_stash);
	}
	free (stash);
...
char	*process_file(int fd, char *stash)
{
	...
	if (buff == NULL)
		return (NULL);
	bytes_read = 1;
	while (!ft_ft_strchr(&stash[ft_len(stash) - bytes_read], '\n')
		&& bytes_read)
	{
